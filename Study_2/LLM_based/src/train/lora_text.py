"""
LoRA fine-tuning for the text AI-detection dataset.
"""
...
DEFAULT_CONFIG_PATH = PROJECT_ROOT / "configs" / "lora_text.yaml"
DATASET_NAME = "text_ai_detection"
DATASET_SUBDIR = "dataset_text"
...
def build_train_text(example: Dict, tokenizer) -> str:
    ...
    assistant = example.get("assistant") or ""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="LoRA fine-tuning for text AI-detection.")
    ...
    if not model_name:
        raise SystemExit("model_name missing; set it in base.yaml or lora_text.yaml.")

    output_dir = Path(train_cfg.get("output_dir", PROJECT_ROOT / "outputs" / "lora_text"))
    ...
    if not split_path.exists():
        raise SystemExit(f"Split dataset not found at {split_path}. Run datasetCreate_textdata.py first.")
